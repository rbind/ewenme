---
date: "2017-06-24"
title: "FPL Mythbusting: Introducing fplR"
author: "Ewen Henderson"
showonlyimage: false
draft: false
image: "blog/img/Qul9n6x.jpg"
weight: 0
type: "post"
description: "Dissecting some FPL with my new package, fplR."
---
...
load("../data/fplR.RData")

```

<br>

I recently authored my first R package (!). First, a quick PSA for anyone thinking about dabbling in the package game - below is a shoutout of the invaluable resources I found along the way that kept me nice and sane.

<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">ease in <a href="https://t.co/NdRYGE1whA">https://t.co/NdRYGE1whA</a><br>more detail <a href="https://t.co/ypqotSAF46">https://t.co/ypqotSAF46</a><br>best practice <a href="https://t.co/qRMZDXBdfQ">https://t.co/qRMZDXBdfQ</a><br>travis 💀 <a href="https://t.co/dEvM3PWsVb">https://t.co/dEvM3PWsVb</a></p>&mdash; ewen (@ewen_) <a href="https://twitter.com/ewen_/status/872480326890795009">June 7, 2017</a></blockquote>
<script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>

<br>

In short, I started on my package contribution journey by focusing on something that I myself would find useful, ensuring at least one reasonably happy user. I play an edition of fantasy sports called [Fantasy Premier League](https://fantasy.premierleague.com/) (FPL), the official fantasy football game of Engand's top division. If you really want to know the in's and out's of FPL, there's a [section of the site](https://fantasy.premierleague.com/a/help) just for rules. TL;DR: for 38 rounds (called 'gameweeks'), pick football players that do good things to score you points and finish high up the leaderboard (rankings).[^definitions]

It so happens that we're in the off-season at the moment, and I figured I could use this down-time to explore how R might assist with my exploits in the coming season.[^data]

```{r out.width = "80%", fig.align = "center"}
knitr::include_graphics("https://raw.githubusercontent.com/ewenme/fplR/master/fplHex.png")
```

Introducing...[`fplR`](https://ewenme.github.io/fplR/)! A package for the strange intersection of FPL / R users like me to explore the game's data. I want to just test drive it here by pulling out some insights concerning my most recent season (2016/2017), and then exploring some of the mythical quirks of FPL as a bit of fun.[^fullcode]

<br>

## How did I do?

As the game updates each user's scores following the day's matches, the site helpfully tells you your rankings (overall, in your country, in any mini-leagues you are part of, etc.). Of course, just knowing your performance at the finish line does not begin to capture the season's journey. I'm going to dive into `fplR` to see if it can help illuminate performance over time (read: rollercoaster of emotions and deep suffering).
...
ggplot(data = myPerformance, aes(x=event, y=overall_rank, group=1)) +
  ...
  labs(title="My Overall Rank across the 2017/18 FPL Season",
       x="Gameweek no.",
       y="Overall Rank")
```

The sharp decline in my ranking around gameweek #5 looks striking, as is the fightback to something respectable in gameweeks 7-10. Nevertheless, my final rank was a personal best of 4,752nd in the world. (I hope that establishes my credentials, nice and early). Bigger variations in the earlier gameweeks are to be expected - the sample size is small, therefore the pack is close, so to speak.

<br>

## FPL mythbusting

Next, I'll examine some things that FPL users have seemingly pondered on since the beginning of time. What can I do to dispel/confirm such community beliefs?

<br>

### The early season rat race

Many users emphasise the importance of a good start, claiming that it is more difficult to do well in the latter stages of the season without a solid foundation built early on. To illustrate this, let's look at the average shift in overall rank by gameweek in one of my mini-leagues (a smaller league amongst a peer group) as the season progresses i.e. how much does a users rank change from week-to-week (N.B. in the current package version, I haven't yet developed a function to help make this easier. Soon come.)?
...
leagueDetailed %>%
  ...
  labs(title="Mean Change in Rank across the Season",
       ...
       caption="Source: https://fantasy.premierleague.com/a/leagues/standings/458675/classic")

```

The "elbow" in this chart comes at a similar time to when my rank levelled off somewhat. You might've noticed the spike in the penultimate gameweek - in this instance, lots of players actually played twice (known as a "double gameweek" to insiders), meaning a greater opportunity for big swings in user rankings.

<br>

### Team value

Another factor perceived to be important in gaining a healthy position for high scores is that of team value. As football players in the game perform well (or not so well), their price changes as a function of market forces i.e. how many users are transferring them into/out of their team? Therefore, perhaps we would expect someone's final team value to be a good predictor of their final rank (N.B. one outlier was removed from this plot - sometimes people chase the team value as a game in itself, incurring points penalties that drastically affect your team's points total. This was one of those cases).
...
Every user remembers the time they left that weeks hot player on the bench (each gameweek, you must choose a starting 11 players - if they play, your back-up players, or 'bench', do not play and do not collect points). Much of the time, this leads to disgruntled folk who hold dear the belief that the "FPL Gods" are consistently against them. If we look at users' benches across the season, how much variation is there?
...
leagueDetailed %>%
  ...
  labs(title="No. of Points Benched over the Season",
       subtitle="Sample of (364) 'active' users taken from an FPL mini-leage",
       x="Total no. of benched points",
       y="Count",
       caption="Source: https://fantasy.premierleague.com/a/leagues/standings/458675/classic")
```

What we see is close to a normal distribution. So, probably no FPL God's retribution, guys. Most people benched somewhere in the region of 150-300 points over the course of a season, with an unlucky/lucky few coming in higher or lower respectively.

<br>

### 'Bandwagons'

Another phenomenon, representing something of an FPL meta-game, is that of the "bandwagon". This refers to times when players put in particularly explosive performances, triggering big movements in the market (i.e. lots of people bringing them in). When were the biggest transfer booms?

```{r}
playersDetailed %>%
  group_by(player_id) %>%
  mutate(var=paste(web_name, "(vs.", lag(opponent_team), ")")) %>%
  ungroup() %>%
...
As for the biggest net losses, these mostly belonged to players of high ownership who suffered serious injuries/suspensions. The most interesting case is that of Diego Costa, who was the centre of a media frenzy claiming a (real-life) transfer to China was imminent (this never materialised).

Of course, these absolute swings are largely governed by a player's intial ownership size and profile. We can look at percentage changes in ownership of players to consider those who had large *relative* shifts in selection. 

```{r}
playersDetailed %>%
  group_by(player_id) %>%
  mutate(selected_chg = ((selected - lag(selected))/lag(selected)),
         var=paste(web_name, "(vs.", lag(opponent_team), ")")) %>%
...
Of course, the % drop in ownership is capped (ownership can drop 100%, max). Interestingly, just two games after Jesus' huge upswing in ownership, he gave us the biggest fall (almost 90% of owners dropped him)! If that's not FPL's Murphy's Law in full effect, I don't know what is.

<br>

### When were the top players hot (or not)?

Come the end of the season, we check the top point-scoring players and yearn for what might've been. How much love did the top (eight) overall point scorers actually get from FPL users over the season?
...
I hope this illustrates some of the possibilities available with the `fplR` package. I had a lot of fun making it, and look forward to using it to better understand my FPL experience. Do share any interesting things you're able to do with it (or any things that go wrong with it...), and enjoy the new season.

<br>

[^definitions]: In this post I have referred to actual footballers as 'players', and players of the FPL game as 'users'.
[^data]: At current, the FPL site only keeps detailed data for the *current* season. Therefore, some of the data used in this analysis will likely be unavailable once the new FPL season begins. For access to all data used in this analysis, please go [here](https://github.com/rbind/ewenme/blob/master/content/data/fplR.RData).
[^fullcode]: To keep the post concise I don't show all of the code, especially code that generates figures. But you can find the full code [here](https://github.com/rbind/ewenme/blob/master/content/blog/2017-06-19-FPL_mythbusting.Rmd).