...
date: "2017-11-06"
type: "post"
tags: ["R", "Linear Programming", "Linear Optimization", "Fantasy Football"]
output:
  html_document:
    fig_caption: yes
---
...
```{r route_plan, out.width = "100%", fig.align = "center", echo=FALSE, fig.cap="You want this journey to work, don't you."}
...
```{r optim_cartoon, out.width = "100%", fig.align = "center", echo=FALSE, fig.cap="HGV transportation optimization: is there anything an animated cartoon can't make look fun?"}
...
```{r wrong_rotation, out.width = "100%", fig.align = "center", echo=FALSE, fig.cap="Not this kind of segue."}
...
unique_goalies %>%
  ...
  datatable(rownames = FALSE,
            caption = 'FPL goalkeepers, sorted by age')
```
...
goalie_data %>%
  ...
  labs(x="Gameweek", y=NULL, title="Goalkeeper Scoring Patterns", subtitle="Courtois vs. Cech, Gameweeks 1-10", caption="made by @ewen_") +
  scale_x_continuous(breaks = seq(1, 10, 1)) +
  # aesthetics
  scale_fill_viridis_c(option = "A", guide = guide_colourbar(title = "Points", direction = "horizontal")) +
  coord_equal() +
  theme_ipsum() +
...
goalie_rotation %>%
  ...
  top_n(10, wt=points_per_mil) %>%
  arrange(desc(points_per_mil)) %>%
  mutate(points_per_mil=round(points_per_mil, digits = 1)) %>%
  select(Combo=combo, Points=points, Cost=total_price, `Points per million`=points_per_mil) %>%
  # formattable styling
  mutate(`Points per million`=color_bar("lightgreen")(`Points per million`),
         Cost=color_tile("white", "orange")(Cost)) %>%
  # kable table
  kable("html", caption = "Top goalkeeper combos (optimized on points per million)", escape = F) %>%
  kable_styling()
...
goalie_rotation %>%
  ...
  mutate(`Points per million`=color_bar("lightgreen")(`Points per million`)) %>%
  # kable table
  kable("html", caption = "Top goalkeeper combos for 9 million or less (optimized on points per million)", escape = F) %>%
  kable_styling()
...
[^fullcode]: To keep the post concise I don't show all of the code, especially code that generates figures. But you can find the full code [here](https://github.com/rbind/ewenme/blob/master/content/blog/2017-11-06-Linear_Programming_Fantasy.Rmd).
