...
The [`geofacet`](https://github.com/hafen/geofacet) package dropped recently, a `ggplot2` extension for faceting plots in a way that retains some underlying geographical dimension. It's perhaps a little bit more abstract than [tilegrams](https://pitchinteractiveinc.github.io/tilegrams/), but the visualizations we produce can be a bit more complex as well. This post basically documents my first foray into `geofacet` functionality, while learning some more about the geographic evolution of London's nightlife in the process.[^fullcode]

## Getting to grips with grids

Via [faceting in `ggplot2`](https://ggplot2.tidyverse.org/reference/facet_wrap.html), we are already familiar with the idea of placing a sequence of plots into a number of panels, possibly arranged according to a variable(s) of interest.

`geofacet` lets you arrange a sequence of plots to preserve some known geographical orientation. These known geographical orientations are stored as **grids**, which provide us with row and column values corresponding to a panel's position in a faceted plot. Take this US states example, providing grid positions for US states:

```{r grid eg}
head(geofacet::us_state_grid1)
```

This grid also contains geographical data (state codes/names). We can simply join data (that sits nicely within this geographical framework) to this grid and start 'geofaceting'.
...
knitr::include_graphics("/img/2017-05-30-geofacet_ldn_pubs/boroughs.png")
```

With that cleared up, let's load the data and get it into shape.
...
ggplot(data = pubs, aes(x=year, y=yoy_chg, fill=yoy_chg)) +
  ...
  scale_fill_viridis(option="D",limits=c(-0.35,0.25), name="Year-on-year\n% change",
                     labels=percent) +
  theme(legend.position=c(.9, .95), strip.text = element_text(size = 8))
...
[^fullcode]: To keep the post concise I don't show all of the code, especially code that generates figures. But you can find the full code [here](https://github.com/rbind/ewenme/blob/master/content/blog/geofacet-london-pubs.Rmd).