---
title: "Diggin' into Hardwax & Honest Jons' reviews"
date: '2019-07-15'
tags:
  - music
  - r
description: "A quantitative appreciation of the inimitable wordplay over at two of my favourite record stores' sites. TIP!"
slug: diggin-into-hardwax-honest-jons-reviews
image: "/blog/2019-07-15-diggin-into-hardwax-honest-jons-reviews_files/figure-html/reggae-cat-odds-1.png"
draft: no
---
...
# load cran pkgs
pacman::p_load("tidyverse", "tidytext", "ghibli", "see", "ggforce", "ggrepel", "scales")
...
ggplot(review_word_counts, aes(x = word_count, fill = store, colour = store)) + 
  ...
  labs(title = "Record store review lengths",
...
Sticking with Honest Jons' reviews for a bit - the most common and specific words have been explored above, but examining these concepts in tandem can help paint a better picture of what makes an HJ review.[^ggforce]
...
word_log_odds %>% 
  ...
  theme_ewen_ws(axis = FALSE, grid = "XY", plot_title_size = 16, subtitle_size = 12, 
                base_size = 12) +
  labs(title = "Honest Jons specific / frequent review words",
       subtitle = "The most frequently occurring and most characteristic words in Honest Jons\nrecord reviews. Top 50 most frequent-occuring words included.",
...
ggplot(hj_review_word_counts, aes(x = fct_reorder(category, -word_count), y = word_count)) + 
  geom_sina(alpha = 0.5, colour = ghibli_palettes$KikiMedium[7], size = 0.5) +
  scale_y_continuous(expand = c(0, 0)) +
  theme_ewen_ws(plot_title_size = 18, subtitle_size = 12, base_size = 14) +
  theme(axis.text.x = element_text(size = 10)) +
  labs(title = "Honest Jons review lengths",
       subtitle = "The word counts of Honest Jons reviews, by site category. Ordered (descending,\nleft to right) by median review length.",
...
Interestingly, *Reggae* has the highest absolute log odds, suggesting that words used in reggae reviews are more "uniquely reggae" than in other listed categories. Beyond the top ten, what does the whole picture look like?
...
hj_log_odds %>% 
  ...
  geom_point2(colour = ghibli_palettes$KikiMedium[7], size = 3, alpha = 0.5) + 
  geom_mark_ellipse(aes(filter = count_per_review > 0.1, description = desc),
                    label.fontsize = 9, label.family = "Work Sans Light") +
  theme_ewen_ws(axis = FALSE, grid = "XY", plot_title_size = 16, subtitle_size = 12, 
                base_size = 12) +
  labs(title = "Reggae specific / frequent review words",
       subtitle = "The frequency and specificity of words used in Honest Jons'\nreggae record reviews.",
       x = "Words per review", y = "Weighted log odds ratio", caption = "@ewen_") 
```
...
[^fullcode]: To keep the post concise and pretty for non-coders, I don't show all of the code, but you can find all the code [here](https://github.com/rbind/ewenme/blob/master/content/blog/2019-07-09-diggin-into-hardwax-honest-jons-reviews.Rmd).
