...
club_names <- c("Manchester City"="Man City", "Manchester United"="Man Utd", 
                "Wolverhampton Wanderers"="Wolves", "Newcastle United"="Newcastle",
                "Tottenham"="Spurs")

# replace club name aliases
epl_player_stats$team_name <- str_replace_all(epl_player_stats$team_name, pattern = club_names)
...
epl_player_stats$web_name[epl_player_stats$player_name == "Lucas Moura"] <- "Lucas Moura"
epl_player_stats$web_name[epl_player_stats$player_name == "Miguel Almirón"] <- "Almirón"
epl_player_stats$web_name[epl_player_stats$player_name == "Salomón Rondón"] <- "Rondón"
epl_player_stats$web_name[epl_player_stats$player_name == "Ayoze Pérez"] <- "Pérez"
epl_player_stats$web_name[epl_player_stats$player_name == "Johann Berg Gudmundsson
"] <- "Gudmundsson"

# fuzzy join understat/fpl player data
player_stats <- stringdist_left_join(
  select(fpl_data, -photo, -team_code:-news, -news_added:-value_season,
         -in_dreamteam, -dreamteam_count, -form:-loaned_out, 
         -ep_this:-special, -own_goals:-team), 
  select(epl_player_stats, -year, -yellow_cards:-position), 
  by = c("web_name", "team_name"), max_dist = 2
  )
