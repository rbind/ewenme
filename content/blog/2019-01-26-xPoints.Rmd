...
Let's get some of these numbers on the board, already. I'll start with a simple scatterplot of xG90 vs xA90, and also mark out players' FPL positions (PS - the chart styles come courtesy of another personal pkg, [ewenthemes](https://github.com/ewenme/ewenthemes)).
...
ggplot(player_stats, aes(x = xG_90, y = xA_90)) +
  ...
  # add chart labels
  labs(title = "Expected goals contributions, 2018/19",
       subtitle = "Expected goals (xG) and expected assists (xA) per 90 minutes. Positions as defined in Fantasy Premier League (FPL).\nPlayers with less than 900 minutes removed.",
  ...
  # add my chart theme
  theme_ewen_ws(grid = FALSE, subtitle_size = 10) +
  theme(legend.position = "top", legend.title = element_blank())

```

This is *starting* to look useful for an FPL manager. It's possible to spot players who are regularly creating and getting goalscoring opportunities. A bit more work is needed to identify who the best picks are.

## Speaking the same language

First, it's probably more helpful to express these expected contributions in terms that are familiar to FPL managers, and take account of the different rewards based on player position. 

```{r xPoints-calc, echo=FALSE}
library(ghibli)
...
player_stats %>% 
  ...
  coord_flip() +
  # add chart labels
  labs(title = "Points from expected goals contributions, 2018/19",
       subtitle = "Top 20 expected FPL points (xP) from goals and assists per 90 minutes. Players with less than 900 minutes removed.",
       x = NULL, y = "xP per 90", caption = "source: understat.com, fantasy.premierleague.com | @ewen_") +
  # add my chart theme
  theme_ewen_ws(grid = "X", axis = FALSE, axis_text_size = 9, subtitle_size = 10) +
...
player_stats %>% 
  ...
  coord_flip() +
  # add chart labels
  labs(title = "Points per £m from expected goals contributions, 2018/19",
       subtitle = "Top 20 expected FPL points per £m (xPm) from goals and assists per 90 mins. Players with less than 900 minutes removed.",
