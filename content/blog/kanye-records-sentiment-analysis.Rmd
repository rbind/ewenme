...
date: "2017-03-20"
tags: ["r", "sentiment-analysis", "kanye-west", "genius"]
---
...
library(RCurl); library(ggplot2); library(hrbrthemes); library(tidytext); library(tidyverse); library(forcats); library(ggalt); library(broom); library(stringr); library(httr); library(jsonlite); library(rvest)
...
```{r ETL, include=FALSE, eval=FALSE}
...
[^fullcode]: To keep the post concise I don't show all of the code, especially code that generates figures. But you can find the full code [here](https://github.com/rbind/ewenme/blob/master/content/blog/kanye-records-sentiment-analysis.Rmd).
