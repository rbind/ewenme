...
eng_birthdays %>% 
  ...
  group_by(age) %>% 
  summarise(mean_day = mean(as.numeric(cutoff_bday_interval))) %>% 
  ggplot(aes(x = age, y = mean_day)) +
  geom_line() +
  geom_point() +
  scale_y_reverse(limits = c(365, 0)) +
  scale_x_reverse() +
  labs(title = "", x = "Player age", y = "# days") +
  theme_ewen_ws()
```
