...
# remotes::install_github("ewenme/ewenthemes")
...
# remove track part cues from lyrics
...
# transform data into one row per word
tidy_df <- df %>%
  unnest_tokens(word, lyrics) %>%
  # remove common ad-libs
  filter(!word %in% c("la", "uh"))

#censor some things
tidy_df$word <- str_replace_all(
  tidy_df$word, c("nigga" = "n**ga", "niggas" = "n**gas",
                  "shit" = "s**t", "fuck" = "f**k")
  )
...
# chart caption
chart_caption <- "lyric data from genius.com | @ewen_"
```
...
tidy_df %>%
  ...
  geom_lollipop(point.colour = ghibli_palettes$SpiritedMedium[4], point.size = 3, horizontal = TRUE) +
  scale_x_comma(breaks = seq(0, 15000, by=2500), limits = c(0, 15000), expand = c(0, 0)) +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) +
  theme_ewen_ws(axis = "y", base_size = 11, plot_title_size = 16) +
  theme(axis.text.y = element_text(margin = margin(r = 10), size = 10)) +
...
```{r}
album_word_counts <- tidy_df %>%
  count(n(), album) 

album_data <- df %>%
  group_by(album) %>%
  summarise(duration=sum(duration)) %>%
  left_join(album_word_counts, by="album") %>%
  mutate(count_per_sec=n/duration)

ggplot(album_data, aes(x = count_per_sec, y = fct_rev(album))) +
  geom_lollipop(point.colour = ghibli_palettes$SpiritedMedium[4], point.size = 3, horizontal = TRUE) +
  scale_x_continuous(limits = c(0, 3), expand = expand_scale(mult = c(0, .1))) +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) +
  theme_ewen_ws(axis = "y", base_size = 11, plot_title_size = 16) +
  theme(axis.text.y = element_text(margin = margin(r = 10), size = 10)) +
...
tidy_df %>%
  count(n(), name) %>%
  ggplot() +
  geom_histogram(aes(n), fill = ghibli_palettes$SpiritedMedium[4]) +
  scale_y_continuous(expand = expand_scale(mult = c(0, 0.1))) +
  theme_ewen_ws(base_size = 11, plot_title_size = 16) +
  annotate("text", label = "'Last Call'", x = 2800, y = 2) +
  labs(title = "Distribution of word counts in Kanye West album tracks",
       x = "Word count", y = "No. of tracks",
       caption = chart_caption)
```
...
tidy_df %>%
  ...
  ggplot() +
  geom_histogram(aes(count_per_sec), fill = ghibli_palettes$SpiritedMedium[4]) +
  theme_ewen_ws(base_size = 11, plot_title_size = 15) +
  scale_y_continuous(expand = expand_scale(mult = c(0, 0.1))) +
  annotate("text", label = "'Freestyle 4'", x = 8.3, y = 2) +
  labs(title = "Words-per-second distribution of Kanye West album tracks",
       x = "Words per Second", y = "No. of tracks",
       caption = chart_caption)
```
...
df %>%
  ...
  geom_dumbbell(size = 3, color = "#e3e2e1", 
                colour_x = ghibli_palettes$SpiritedMedium[3], 
                colour_xend = ghibli_palettes$SpiritedMedium[4],
                dot_guide = FALSE) +
  scale_x_continuous(breaks = seq(0, 800, by = 200)) +
  scale_y_discrete(labels = function(x) str_wrap(x, width = 20)) +
  labs(x = "Track length (seconds)", y = NULL, title = "Range of track lengths in Kanye West albums",
       caption = chart_caption) +
  theme_ewen_ws(base_size = 11, plot_title_size = 16)
```
...
sentiment_df %>%
  ...
  geom_lollipop(point.colour = ghibli_palettes$SpiritedMedium[4], point.size=3) +
  scale_y_continuous(expand = expand_scale(mult = c(0, 0.1))) +
  theme_ewen_ws(base_size = 11, plot_title_size = 16) +
...
by_track_sentiment %>%
  ...
  labs(title="Ratio of negative/positive sentiment in \nMy Beautiful Dark Twisted Fantasy",
       x="Sentiment ratio (negative-positive)", y=NULL,
       caption=chart_caption)
```
...
by_track_sentiment %>%
  ...
  geom_lollipop(point.colour=ghibli_palettes$SpiritedMedium[4], point.size=2, horizontal=TRUE) +
  theme_ewen_ws(base_size = 11, plot_title_size = 16, axis_text_size = 8) +
