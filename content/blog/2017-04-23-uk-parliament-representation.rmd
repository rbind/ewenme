---
title: "Representation within UK parliament"
description: "Leveraging open politicians data to learn more about representation and diversity within UK Parliament."
date: "2017-04-23"
tags: ["r", "politics"]
slug: uk-parliament-representation
draft: no
---
...
chart_caption = "source: everypolitician.org | @ewen_"
...
gender_split <- members %>%
  filter(legislative_period_id == "Term 56\n(2015-)" & on_behalf_of_id_recoded %in% c("Conservative", "Labour", "SNP")) %>%
...
iron(
  waffle(c(Female=gender_split[1,2], Male=gender_split[2,2]), rows = 5,
    ...
    scale_fill_ghibli_d(name = "MarnieMedium1"),
...
members %>%
  ...
  theme_ewen_ws() +
  scale_y_percent(limits = c(0, 0.3), breaks = seq(0, 0.3, 0.1), expand = c(0, 0)) +
  labs(title="How many terms did elected MPs get, across the last five\nterms?",
       x="No. of terms", y="% of MPs",
...
members %>%
  ...
  geom_point(size=2) +
  theme_ewen_ws() +
  scale_colour_ghibli_d(name = "MarnieMedium1") +
...
int_members %>%
  ...
  labs(title="Female representation in elected politicians",
       x=NULL, y="% of female seats",
       subtitle="Elected government terms, in chronological order (most recent on the right)",
       caption=chart_caption) +
  scale_y_percent(limits = c(0, 0.6), labels = scales::percent_format(accuracy = 1L)) +
  geom_hline(yintercept = 0.5, colour="firebrick", linetype="dashed") +
...
[^fullcode]: To keep the post concise I don't show all of the code, especially code that generates figures. But you can find the full code [here](https://github.com/rbind/ewenme/blob/master/content/blog/uk-parliament-representativeness.Rmd).
